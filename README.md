...
Un árbol de decisión es un modelo que clasifica datos mediante la realización de preguntas lógicas consecutivas en forma de ramas, estas preguntas se basan en las características del conjunto de dato y terminan hasta que llega a una respuesta final o la pureza del nodo es de 100%, significando que ya no puede realizar más preguntas.  
Este modelo se distingue por no requerir el escalamiento de los datos ni el uso de encoding para transformar las cadenas de caracteres.  En general, es un modelo bastante intuitivo; sin embargo, es susceptible al overfiting si no tiene un control adecuado del crecimiento de sus ramas.
